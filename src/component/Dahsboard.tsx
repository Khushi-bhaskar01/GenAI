...
const Dashboard: React.FC<ProfilePanelProps> = ({
  ...
  // Handle navigation with fallback to original callback
  const handleNavigation = (route: string, callback?: () => void) => {
    try {
      navigate(route);
      onClose(); // Close the panel after navigation
    } catch (error) {
      // If navigation fails, fall back to original callback
      callback?.();
    }
  };
  ...
  const menuItems = [
    {
      ...
      label: "Dashboard",
      onClick: () => handleNavigation("/", onDashboard),
      route: "/dashboard",
    },
    {
      icon: Monitor,
      label: "Skill Monitor",
      onClick: () => handleNavigation("/skill", onSkillMonitor),
      route: "/skill-monitor",
    },
    {
      icon: BookOpen,
      label: "Course Library",
      onClick: () => handleNavigation("/", onCourseLibrary),
      route: "/course-library",
    },
    {
      icon: Map,
      label: "Roadmap",
      onClick: () => handleNavigation("/roadmap", onRoadmap),
      route: "/roadmap",
    },
    {
      icon: FileText,
      label: "Assessments",
      onClick: () => handleNavigation("/assessment", onAssessments),
      route: "/assessment",
    },
    {
      icon: File,
      label: "Resume",
      onClick: () => handleNavigation("/resume", onResume),
      route: "/resume",
    },
    {
      icon: Settings,
      label: "Settings",
      onClick: () => handleNavigation("/", onSettings),
      route: "/settings",
    },
  ];
